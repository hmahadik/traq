...
func TestBackfill_AssignsAISessionsByProjectDir(t *testing.T) {
	dir, err := os.MkdirTemp("", "traq-backfill-ai-*")
	if err != nil {
		t.Fatalf("mkdir: %v", err)
	}
	defer os.RemoveAll(dir)
	...
	if err := store.UpsertAISession(sess); err != nil {
		t.Fatalf("UpsertAISession: %v", err)
	}
	...
	got, err := store.GetAISessionByID("test-session-1")
	if err != nil {
		t.Fatalf("GetAISessionByID: %v", err)
	}
	if got == nil {
		t.Fatal("ai session not found")
	}
	if !got.ProjectID.Valid || got.ProjectID.Int64 != proj.ID {
		t.Errorf("ai session project_id = %v, want %d", got.ProjectID, proj.ID)
	}
}
